...
#   python weather_ops.py download --pjm-glob "data/raw/pjm/hrl_load_metered_*.csv" --zones-json config/zone_coords.json --out data/raw/noaa_hourly.csv
#
#   # B) Split a huge weather CSV into 10 files:
...
def split_weather(in_csv: str, outdir: str, overwrite: bool):
    """
    Split into 10 files:
      - weather_2017.csv … weather_2024.csv  (first 9 full years)
      - weather_2025_jan_oct.csv
      - weather_2025_nov.csv
    ...
    targets = {year: os.path.join(outdir, f"weather_{year}.csv") for year in range(2018, 2025)}  # 2018..2024
